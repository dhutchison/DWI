...
Script to add all twitter favourites for a user into the default list in 
the Reminders app under OS X 10.8 Mountain Lion.
...
reminders=Appscript.app('Reminders')
begin
  wasRunning = false
  reminders.run
end unless reminders.is_running?
  ...
  begin
    ...
    unless (page == nil || page.empty?)
      page.reverse.each { |tweet|
        ...
        reminders.default_list.make(:new => :reminder, :with_properties => {
        	:name => "Tweet from #{tweet.user.name} http://twitter.com/#{tweet.user.screen_name}/statuses/#{tweet.id} #{context}",
        	:body => "#{tweet.text}",
        })
...
puts "Successfully imported #{importedTweets} tweets into Reminders"
...
#If Reminders was not running when this script started, then quit it again
reminders.quit if !wasRunning
